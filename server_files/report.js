...
var arrayAccountsTxt = fs.readFileSync("accounts_report/accounts_" + accounts_list_id +".txt").toString().split("\n");
        ...
        SteamGCs[indexElement].on("message", function(header, buffer, callback) {
            switch (header.msg) {
                ...
                case Protos.ECsgoGCMsg.k_EMsgGCCStrike15_v2_ClientReportResponse:
                    CountReports++;
					console.log("[" + prefix + " - (" + CountReports + ")] Report with confirmation ID: " + Protos.CMsgGCCStrike15_v2_ClientReportResponse.decode(buffer).confirmationId.toString() + " sent!");
					if (CountReports == size)
					{
					console.log("\n\n"+ CountReports + " Reports for this faggot.\nThanks for using this Service!\nCredits for the Script to askwrite & TROLOLO");
					}
...
if ( report_matchid != 8){
    console.log("Using MatchID: " + report_matchid + "\n");
} else {
